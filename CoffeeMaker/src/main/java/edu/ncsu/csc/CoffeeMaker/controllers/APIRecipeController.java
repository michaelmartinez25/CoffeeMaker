...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity editRecipe ( @RequestBody final Recipe updatedRecipe, @PathVariable final String name ) {
    	final Recipe currRecipe = service.findByName( name );
    	...
    	if (updatedRecipe == null) {
    		return new ResponseEntity (errorResponse( "Requested recipe cannot be null" ), HttpStatus.BAD_REQUEST);
    	}
    	if (updatedRecipe.getPrice() < 0) {
    		return new ResponseEntity (errorResponse( "Requested recipe's price cannot be negative" ), HttpStatus.BAD_REQUEST);
    	}
    	if (updatedRecipe.getIngredients().size() < 1) {
    		return new ResponseEntity (errorResponse( "Requested recipe must have at least one ingredient" ), HttpStatus.BAD_REQUEST);
    	}
    	for (Ingredient i : updatedRecipe.getIngredients()) {
    		if (i.getAmount() < 0) {
    			return new ResponseEntity (errorResponse( "Requested recipe's ingredients cannot have negative quantities" ), HttpStatus.BAD_REQUEST);
    		}
    	}
    	
    	service.update( name, updatedRecipe );
